...
from src.utils import get_transform_from_file, get_sox_effect_from_file, digital_filter, pad_waveform
...
class SPEECHCOMMANDS(SPEECHCOMMANDS):
    ...
    def __getitem__(self, n: int):
        ...
        # convert label to integer, note that abnormal is 0, normal is 1
        label = self.class_to_idx['normal'] if label == self.normal_class else self.class_to_idx['abnormal']
        # convet range to 0~1
...
class AudioFolder(DatasetFolder):
    def __init__(self, root: str, project_parameters, stage, transform=None):
        super().__init__(root, loader=None, extensions=('.wav'), transform=transform)
        self.project_parameters = project_parameters
        self.transform = transform
        self.stage = stage
    ...
    def __getitem__(self, index: int):
        ...
        label = self.class_to_idx[filepath.rsplit('/', 2)[-2]]
